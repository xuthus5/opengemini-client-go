...
import (
	...
	"github.com/openGemini/opengemini-client-go/internal/record"
...
func (r *RowsWriterClient) Flush(ctx context.Context, database, retentionPolicy string) error {
	...
	conn, err := grpc.NewClient("127.0.0.1:8305", opts...)
	if err != nil {
	...
	response, err := client.Write(ctx, req)
	if err != nil {
...
func NewRPCClient(cfg *Config) *RowsWriterClient {
...
func (c *columner) processTimestamp(tt time.Time) (err error) {
	...
	if !exists {
		timeCol, err = c.createColumn(TimeColumnName, record.FieldTypeInt)
		if err != nil {
			return err
		}
	}
